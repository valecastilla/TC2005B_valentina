...
class Ball extends GameObject {
    ...
    initVelocity() {
        ...
        this.velocity = new Vec(Math.cos(angle), Math.sin(angle)).times(
            initialSpeed
        );
...
let barwidth = 50;
const bar = new Bar (new Vec(canvasWidth / 4 * 3, canvasHeight / 4), barwidth, 10, "white");

// An object to represent the ball to be displayed
const box = new Ball(
    new Vec(canvasWidth / 2, (canvasHeight / 4) * 3),
    10,
    10,
    "white"
);
...
const paddle = new Paddle(
    new Vec(canvasWidth / 2 - 50, (canvasHeight / 4) * 3 + 10),
    100,
    20,
    "white"
);

const upperBorder = new GameObject(
    new Vec(0, 0),
    canvasWidth,
    10,
    "white",
...
const downBorder = new GameObject(
...
const leftBorder = new GameObject(
...
const labelLife = new TextLabel(
    canvasWidth - 175,
    50,
    "40px Ubuntu Mono",
    "white"
);
const labelGameOver = new TextLabel(
    canvasWidth / 8,
    canvasHeight / 2 + 100,
    "50px Ubuntu Mono",
    "rgb(255, 184, 224)"
...
function drawScene(newTime) {
    ...
    if (boxOverlap(box, paddle)) {
        const hitPosition =
            (box.position.x + box.width / 2 - paddle.position.x) / paddle.width;
